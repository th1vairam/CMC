...
Data.Files = synQuery('select * from file where projectId=="syn3455058" and fileType == "rda"', blockSize = 100)
Data.Files = Data.Files$collectAll()
Module.Files = synQuery('select * from file where projectId=="syn3455058" and fileType == "tsv" and moduleMethod == "igraph:fast_greedy"', blockSize = 100)
Module.Files = Module.Files$collectAll()
Module.Files = Module.Files[is.na(Module.Files$file.enrichmentMethod),]

UModule.Files = Data.Files[!(paste(tools::file_path_sans_ext(Data.Files$file.name),Data.Files$file.disease) %in%
